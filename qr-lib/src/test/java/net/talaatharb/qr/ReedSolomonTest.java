...
class ReedSolomonTest {
	...
	void testCreateIntMessage() {
		byte[] data = new byte[] {(byte) Integer.parseInt("00110011", 2),
		(byte) Integer.parseInt("11100011", 2)};
	...
	void testGenerateErrorCorrectionCodewords() {
		Integer codeWordCount = QRGenerator.CODE_WORD_COUNT;
		byte[] data = new byte[] {(byte) 51,
				(byte) -29};
		var errorCorrectedData = ReedSolomon.generateErrorCorrectionCodewords(data, codeWordCount);
		
		assertArrayEquals(new byte[] {117, 119, 16, -70, -121, -127, 126}, errorCorrectedData);
	}
